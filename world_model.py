...
class WorldModel:
    ...
    def save_to_disk(self):
        ...
        for node, data in self.graph.nodes(data=True):
            c.execute(
                "INSERT INTO nodes (concept, domain, properties) VALUES (?, ?, ?)",
                (node, data.get("domain", ""), json.dumps(data.get("properties", {}))),
            )
            c.execute("INSERT INTO nodes_fts (concept, domain) VALUES (?, ?)", (node, data.get("domain", "")))

        for u, v, k, data in self.graph.edges(keys=True, data=True):
            c.execute(
                "INSERT INTO edges (id, source, target, relation, timestamp, properties) VALUES (?, ?, ?, ?, ?, ?)",
    ...
    def load_from_disk(self):
        ...
        for row in c.execute("SELECT id, source, target, relation, timestamp, properties FROM edges"):
            self.graph.add_edge(
                row[1], row[2], key=row[0], relation=row[3], timestamp=row[4], properties=json.loads(row[5])
            )
        ...
        try:
            ...
            if path:
                ...
                for i in range(len(path) - 1):
                    ...
                    max_conf = max(
                        (float(d.get("properties", {}).get("confidence", 1.0)) for d in edge_data.values()), default=1.0
                    )
                    confidence *= max_conf
    ...
    def query_truth_at_time(self, source: str, target: str, time_query: float):
        ...
        valid_edges = [
            data for data in edge_data.values() if data.get("timestamp") is not None and data["timestamp"] <= time_query
        ]
        ...
        for a in self.graph.nodes:
            for b in self.graph.successors(a):
                ...
                for c in self.graph.successors(b):
                    ...
                    if self.graph.has_edge(a, c) and any(
                        d["relation"] == "is_not" for d in self.graph.get_edge_data(a, c).values()
                    ):
                        contradictions.append(
                            {
                                "source": a,
    ...
    def search_nodes(self, query: str, limit: int = 10) -> list[dict]:
        ...
        try:
            ...
            rows = c.execute(
                "SELECT concept, domain FROM nodes_fts WHERE nodes_fts MATCH ? LIMIT ?", (f'"{safe_query}"*', limit)
            ).fetchall()
