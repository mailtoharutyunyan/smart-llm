...
collector = TraceCollector(
    raw_dir=os.path.join(ACS_DIR, "acs/training/raw_traces/")
)
...
dataset_builder = DatasetBuilder(
    output_dir=os.path.join(ACS_DIR, "acs/training/datasets/")
)
...
def acs_think(query: str, deep: bool = False) -> str:
    ...
    if result.get("cognitive_mode") in ("DEEP", "STANDARD"):
        ...
        policy.record_outcome(
            query=query, 
            mode=result.get("cognitive_mode", "STANDARD"), 
            success=success, 
            confidence=conf_val
        )
...
@mcp.tool()
def acs_world_model_add(
    concept: str, domain: str, properties: str = "{}"
) -> str:
